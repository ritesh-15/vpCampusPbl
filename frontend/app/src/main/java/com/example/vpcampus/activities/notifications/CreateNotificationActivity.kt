...
class CreateNotificationActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        notificationViewModel.createNotificationResponse.observe(this) { response ->
            parseCreateNotificationResponse(response)
        }
    ...
    private fun parseCreateNotificationResponse(state: ScreenState<CreateNotificationResponse>) {
        ...
        when (state) {
            ...
            is ScreenState.Success -> {
                hideProgressDialog()
                if (state.data != null) {
                    val json = Gson().toJson(state.data.notification)
                    mSocket?.emit(Constants.NEW_NOTIFICATION, json)
                    Toast.makeText(this, "Notification created successfully!", Toast.LENGTH_SHORT)
                        .show()
                    finish()
                }
            }
    ...
    private fun handleCreateButtonClick() {
        ...
        notificationViewModel.createNewNotification(title,
            description,
